...
function getRectangleString(width, height) {
  const result = [];
  const topEdge = `┌${Array(width - 2).fill('─').join('')}┐\n`;
  result.push(topEdge);
  const middleEdge = `│${Array(width - 2).fill(' ').join('')}│\n`;
  for (let i = 0; i < height - 2; i += 1) {
    result.push(middleEdge);
  }
  const bottomEdge = `└${Array(width - 2).fill('─').join('')}┘\n`;
  result.push(bottomEdge);
  return result.join('');
}
...
function getCardId(value) {
  ...
  const map = new Map();
  cards.forEach((i, index) => map.set(i, index));
